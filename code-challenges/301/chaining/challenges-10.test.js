...
const count = (target, input) => {
  return input.reduce((total, testArr) => {return total + testArr.reduce((total, testNum) => {return total + (testNum === target ? 1 : 0)}, 0)}, 0)
};
...
const totalSum = (input) => {
  return input.reduce((total, testArr) => {return total + testArr.reduce((total, num) => {return total + num}, 0)}, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let output = [];
  input.map(componentArr => {
    let filteredArr = componentArr.filter((element) => {return (typeof element === 'number' && element % 5 === 0)})
    output.push(filteredArr.map((element) => {return Math.pow(2, element)}));
  });
  return output;
};
/* ------------------------------------------------------------------------------------------------
...
let findMaleAndFemale = (data) => {
  return data.filter((character) => {
    ...
    return (output + (output ? " and " : "") + character.name)
  }, "")
}
