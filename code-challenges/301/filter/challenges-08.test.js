...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(thing => thing.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  return arr.filter(thing => thing.baseStat > minBaseStat).map(stat => stat.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  return arr.filter(num => Number.isInteger(num)).map(num => num % 2? 'odd' : 'even');
};
