...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach( (ingred)=> {
    ingred = ingred.slice(ingred.indexOf(' ', ingred.indexOf(' ') + 1) + 1);
    output.push(ingred);
  })
  return output;
};
...
//     return arr;
//   }
//   return removeEvenValues(arr, index + advance);
// };


// does not modify in place, works otherwise
...
//   }
//   if(!(arr[0] % 2)) {
//     return removeEvenValues(arr.slice(1));
//   }
...
const totalSumCSV = (str) => {
  ...
  values.forEach(num => total += parseInt(num));
  return total;
};
...
const extractVowels = (str) => {
  return [str.split(/[aeiou]/).join(''), str.match(/[aeiou]/g).sort( (a, b) => {return a > b}).join('')];
};
