...
const hasChildrenValues = (arr, character) => {
  return Object.values(arr[(arr.map((char) => {return char.name})).indexOf(character)])[2].length;
}
...
const hasChildrenEntries = (arr, character) => {
  return Object.entries(arr[(arr.map((char) => {return char.name})).indexOf(character)])[2][1].length;
}
