...
const countNumberOfElements = (arr) => {
  return arr.reduce( (accumulator, initial) => {
...
const reversedString = (arr) => {
  arr = arr.split('');
  let reversed = arr.reduce( (word, letter) => {
    return letter + word;
  }, "");
  return reversed;
};
...
const countNumberOfChildren = (arr) => {
  return arr.reduce((children, character) => {
    return children + (character.children ? character.children.length : 0);
  }, 0)
};
...
const calculateAverage = (arr) => {
  let listTot = arr.reduce((totals, num) => {
    totals.number++;
    totals.sum += num;
    return totals;
  }, {sum: 0, number: 0})
  return listTot.sum / listTot.number;
};
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((primes, test) => {
    return primes + (isPrime(test)? 1 : 0);
  }, 0)
}
...
const extractStat = (statName, arr) => {
  return arr.reduce((stat, test) => {
    if(test.stat.name === statName)
      stat = test;
    return stat;
  }, null)
};
...
const extractChildren = (arr) => {
  arr = arr.filter(character => character.name.includes('a'));
  return arr.reduce( (children, character) => {
