...
let biggerThanLuke = (arr) => {
  return arr.filter(character => character.mass > 77).map(character => {return character.name}).join(' - ');
}
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  return /^https:\/\/\w+\.[A-Za-z]{3}$/.test(url);
}
...
const detectTicTacToeWin = (board) => {
  // check rows
  board.forEach(row => {if(checkTTTCoord(row)) {return true}});
  // check columns
  for(let i = 0; i < 3; i++) {
    if(checkTTTCoord([board[0][i], board[1][i], board[2][i]])) {return true}
  }
  // check diagonals
  if(checkTTTCoord([board[0][0], board[1][1], board[2][2]])) {return true}
  if(checkTTTCoord([board[2][0], board[1][1], board[0][2]])) {return true}
  return false;
}

const checkTTTCoord = (coords) => {
  console.log(coords);
  return coords[0] ===coords[1] && coords[1] === coords[2] && coords[0] !== ' ';
}
