...
const firstLetters = (arr) => {
  return arr.map((word) => {return word.charAt(0)});
};
...
const findHappiness = (arr) => {
  return arr.filter((word) => {return word.includes(':)')})
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map((word) => {
    // return word.replace(/.(\d{3})..(\d{3}).(\d{4})/, (result, p1, p2, p3) => {console.log(result); return [p1, p2, p3].join('')})
    // return word.match(/\d/g).join('');
    return [word.substring(1, 4), word.substring(6, 9), word.substring(10, 15)].join('');
  })
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((item, index) => {return index&1}).join('');
};
...
const allHappy = (arr) => {
  return arr.every((word) => {return word.includes(':)')});
};
...
const findAnything = (arr, target) => {
  return arr.filter((word) => {return word.includes(target)});
};
...
const findEvery = (arr, target) => {
  return arr.every((word) => {return word.includes(target)});
};
...
const unenrollBrook = (arr) => {
  return arr.map((course) => {return course.filter((student) => {return !student.includes('Brook')})})
};
...
const sortByDay = (arr) => {
  return daysOfWeek.map((day) => {return arr.filter((event) => {return event.includes(day)})});
};
...
const characterByIndex = (arr) => {
  return arr.map((word, index) => {return word.charAt(index)})
};
